...
class GithubController extends BaseController
{
  ...
  public function activate(){
    ...
    $username = 'admin';
    DB::statement('insert into accounts (username,access_token,source_name) values (?,?,?)', array($username, $access_token, 'github'));
      ...
      foreach ($repos as $repo){
        ...
        foreach( $files as $file){
          if ($file['type']=='file' && $file['size']<1000000){
              ...
              $id = null;
              $result = DB::statement('BEGIN articles_package.add_garticle(?,?,?,?,?,?,?); END;', array($username, $repo['name'], $file['path'], $file['name'], $extension, '', 'f'));
